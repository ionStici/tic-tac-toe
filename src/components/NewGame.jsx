...
const NewGame = function (props) {
  ...
  const switchMark = function ({ target }) {
    ...
    setPlayer2(
      active.nextElementSibling?.dataset.mark || active.previousElementSibling?.dataset.mark
    );
  };

  const startNewGame = ({ target }, difficulty = false) => {
    props.startNewGame({
      player1: player1,
      player2: player2,
      currentPlayer: "x",
      ...
      <section className={styles.section}>
        ...
        <div className={styles.panel}>
          ...
          <div className={styles.wrapper}>
            <div className={`${styles.box} ${styles.active}`} data-mark="x" onClick={switchMark}>
              <svg className={styles.x} width="64" height="64" xmlns="http://www.w3.org/2000/svg">
                {assets.path_x}
              </svg>
            </div>

            <div className={styles.box} data-mark="o" onClick={switchMark}>
              <svg className={styles.o} width="64" height="64" xmlns="http://www.w3.org/2000/svg">
                {assets.path_o}
              </svg>
            </div>
          </div>
        ...
        <div className={styles.buttonBox}>
          <ButtonMain handleClick={chooseGameDifficulty} text="New Game (VS CPU)" color="yellow" />
          <ButtonMain handleClick={startNewGame} text="New Game (VS Player)" color="blue" />
        </div>
      </section>
    </>
  );
